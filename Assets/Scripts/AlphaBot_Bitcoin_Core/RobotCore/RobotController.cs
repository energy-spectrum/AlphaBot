...
namespace AlphaBot_Bitcoin.RobotCore
{
    public class RobotController : MonoBehaviour
    {
        ...
        //«‡ˇ‰ ·‡Ú‡ÂË -  ÓÎË˜ÂÒÚ‚Ó ‰ÂÈÒÚ‚ËÈ
        private int battery—harge;

        //ÕÛÊÌÓ Á‡ÌÂÒÚË ‚ ÓÚ‰ÂÎ¸Ì˚È ÍÎ‡ÒÒ, ‰Îˇ ‚ÒÂı Ó·ÓÚÓ‚ ˜ÚÓ· ·˚Î ‰ÓÒÚÛÔ

        private Vector3 _localPositionRobot;

        private Directions direction;

        private int number—ollected—oins = 0;

        private Dictionary<short, List<string>> functions;
        public void Initialization(List<string> allCode, int battery—harge)
        {
            _localPositionRobot = transform.localPosition;

            number—ollected—oins = 0;

            direction = startDirection;

            this.battery—harge = battery—harge;
                        ...
                        switch (allCode[i])
                        {
                            ...
                            case "}":
                                ...
                                if (countBrace == 0)
                                {
                                    ...
                                    isBreak = true;
                                    //“.Í. ¬ÌÛÚ ˆËÍÎ for Ë ‚ÌÂ¯ ‰ÂÎ‡˛Ú i++. “.Â. i += 2, ‡ ÌÛÊÌÓ i += 1
                                    i--;
                                }
            ...
            for (int i = 0; i < codeBlock.Count; i++)
            {
                ...
                switch (codeBlock[i])
                {
                    ...
                    case "TurnRight":
                        ...
                        break;
                    //Jump
                    //ƒÓ‡·ÓÚ‡Ú¸ Ò ËÁÏÂÌÂÌËˇÏË localPosition and direction
                    case "Jump":
                        battery—harge--;
                ...
                if (battery—harge == 0 || IsGameOver.IsGameOver_(_localPositionRobot, number—ollected—oins))
                {
                    _isGameOver = true;
                    return;
                }
